-- 策略函数，即为真时，代表亮起卖出信号
CREATE OR REPLACE FUNCTION s5(symbol CHAR, date DATE)
  RETURNS BOOLEAN AS $$
# 获得最近的3组can
...
# 数据不足
if len(rows) < 3:
    plpy.info('Insufficient data for calculate S5, sql = %s, $1 = %s, $2 = %s' % (sql, symbol, date))
    return None
# 不允许None行
if rows[0]['ca5'] is None or rows[0]['ca10'] is None or rows[0]['ca20'] is None or rows[0]['ca60'] is None or rows[0]['ca120'] is None or rows[0]['ca250'] is None \
    or rows[1]['ca5'] is None or rows[1]['ca5'] == 0\
    or rows[2]['ca5'] is None or rows[2]['ca5'] == 0:
    plpy.info('None or zero value exists, sql = %s, $1 = %s, $2 = %s' % (sql_can, symbol, date))
    return None
# 挡位
gear = rows[0]['ca250'] <= rows[0]['ca120'] and rows[0]['ca120'] <= rows[0]['ca60'] and rows[0]['ca60'] <= rows[0]['ca20'] and rows[0]['ca20'] <= rows[0]['ca10'] and rows[0]['ca10'] <= rows[0]['ca5']
if not gear:
    return False
