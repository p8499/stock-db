-- 现金比率（越小越好）
CREATE OR REPLACE FUNCTION cr(symbol CHAR, date DATE)
  RETURNS NUMERIC AS $$
def cachew(r):
    sql_cachew = 'SELECT cachew($1, $2, $3, $4)'
    plan_cachew = plpy.prepare(sql_cachew, ['CHAR', 'DATE', 'VARCHAR', 'NUMERIC'])
    plpy.execute(plan_cachew, [symbol, date, 'cr', r])
    return r
# 查cache
sql_cacher = 'SELECT yn, val FROM cacher($1, $2, $3)'
plan_cacher = plpy.prepare(sql_cacher, ['CHAR', 'DATE', 'VARCHAR'])
rows_cacher = list(plpy.execute(plan_cacher, [symbol, date, 'cr']))
if rows_cacher[0]['yn']:
    return rows_cacher[0]['val']
# 当前bs.totasset and totliab
# 3月的季报最多用到10月底，6+1个月
# 6月的中报最多用到次年4月底，6+4个月
# 9月的季报最多用到次年4月底，6+1个月
# 12月的季报最多等到明年8月底，6+2个月
sql = 'SELECT "end", curfds, tradfinasset, derifinaasset, totalcurrliab FROM balance_sheet WHERE symbol = $1 AND pub <= $2 AND (' \
...
if len(rows) < 2:
    ...
    return cachew(None)
# 现金资产
cash_asset_0 = rows[0]['curfds'] + rows[0]['tradfinasset'] + rows[0]['derifinaasset']
cash_asset_1 = rows[1]['curfds'] + rows[1]['tradfinasset'] + rows[1]['derifinaasset']
cash_asset = (cash_asset_0 + cash_asset_1) / 2
if cash_asset == 0:
    plpy.info('Cash Asset is zero, symbol = %s, date = %s')
    return cachew(None)
# 流动负债
totalcurrliab = (rows[0]['totalcurrliab'] + rows[1]['totalcurrliab']) / 2
# 计算
return cachew(totalcurrliab / cash_asset)
$$
LANGUAGE plpython3u;