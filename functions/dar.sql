...
sql = 'SELECT "end", totasset, totliab FROM balance_sheet WHERE symbol = $1 AND pub <= $2 AND (' \
...
if totasset == 0:
    ...
    return cachew(None)
# 负债
totliab = (rows[0]['totliab'] + rows[1]['totliab']) / 2
# 计算
return cachew(totliab / totasset)
$$
LANGUAGE plpython3u;