-- 策略函数，即为真时，代表亮起卖出信号
CREATE OR REPLACE FUNCTION sv60(symbol CHAR, date DATE)
  RETURNS BOOLEAN AS $$
# 获得最近的一条can
sql_can = 'SELECT ca20(symbol, eob) AS ca20, ca60(symbol, eob) AS ca60, ca120(symbol, eob) AS ca120, ca250(symbol, eob) AS ca250 FROM history_1d WHERE symbol = $1 and eob <= $2 ORDER BY eob DESC LIMIT 1'
plan_can = plpy.prepare(sql_can, ['CHAR', 'DATE'])
rows_can = list(plpy.execute(plan_can, [symbol, date]))
# 数据不足
if len(rows_can) < 1:
    plpy.info('Insufficient data for calculate SV60, sql = %s, $1 = %s, $2 = %s' % (sql_can, symbol, date))
    return None
# 不允许None行
if rows_can[0]['ca20'] is None or rows_can[0]['ca60'] is None or rows_can[0]['ca120'] is None or rows_can[0]['ca250'] is None:
    plpy.info('Cannot calculate SV60 due to some data are None, sql = %s, $1 = %s, $2 = %s' % (sql_can, symbol, date))
    return None
# 挡位
gear = rows_can[0]['ca250'] < rows_can[0]['ca120'] < rows_can[0]['ca60'] and rows_can[0]['ca20'] <= rows_can[0]['ca60']
if not gear:
    return False
# 构建{eob, close, high, low, volume}数组
sql = 'SELECT eob, close, high, low, volume FROM (SELECT eob, close, high, low, volume FROM history_1d WHERE symbol = $1 AND eob <= $2 ORDER BY eob DESC LIMIT 5 + 1) s1 ORDER BY eob ASC'
...
# 计算
return toprange>= 5 and rows[-1]['close'] > rows[-2]['close']
$$
LANGUAGE plpython3u;

-- select symbol, eob, sv60(symbol, eob) from history_1d where symbol = 'SZSE.300015' and eob between '2018-09-01' and '2018-10-31';
